...
export default function Index() {
  ...
  // useEffect(() => {
  //   if (carts.length > 0) {
  //     setfakeData(fakeUserDetails(carts));
  //   }
  // }, [carts]);
  ...
  useEffect(() => {
    router.beforePopState(async ({ as }) => {
      if (as !== router.asPath) {
        //
        const fakeData = fakeUserDetails(carts);
  ...
  return (
    <div className="min-h-screen w-full bg-white text-black p-8 grid grid-cols-2">
      ...
      <div className="bg-slate-100 h-auto p-10">
        ...
        <button className="mt-3 w-full text-lg bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
          Place order
        </button>
      </div>
    </div>
